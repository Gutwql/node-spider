const rp = require("request-promise"), //进入request-promise模块
    request = require("request"), //进入request-promise模块
    fs = require("fs"), //进入fs模块
    mkdirp = require('mkdirp'),
    cheerio = require("cheerio"), //进入cheerio模块
    depositPath = "./meizitu/"; //存放照片的地址
    ...
    getUrl(data) {
        ...
        $(".all .url a")
            .each(async(i, e) => {
                ...
                let obj = {
                    name: e.children[0].data, //图片网页的名字，后面作为文件夹名字
    ...
    async downloadImage(data, index) {
        if (data.res) {
            ...
            if ($(".main-image").find("img")[0]) {
                ...
                // setTimeout(function() {
                //     downloadPic(imgSrc, `${downloadPath}/${index}.jpg`);
